...
export default Controller.extend({
  ...
  kontainerDrivers: computed('model.kontainerDrivers.@each.{id,state}', function() {
    ...
    const linodeCloud = ['linodecontainerengine'];
    const nope       = ['import', 'rancherkubernetesengine'];
    const kDrivers   = get(this, 'model.kontainerDrivers') || [];
    console.log({kDrivers});
    const builtIn    = kDrivers.filter( (d) => d.state === 'active' && (d.builtIn || chinaCloud.indexOf(d.id) > -1 || oracleCloud.indexOf(d.id) > -1 /*|| linodeCloud.indexOf(d.id) > -1*/) && !nope.includes(d.id));
  ...
  providerChoices: computed('app.proxyEndpoint', 'intl.locale', 'kontainerDrivers.[]', 'model.nodeDrivers.{id,state}', 'schemaReloaded', function() {
    ...
    let out = [
      ...
      },
      {
        name:        'linodelke',
        driver:      'linodelke',
        kontainerId: 'linodecontainerengine'
      },
    ];
    ...
    out.forEach((driver) => {
      ...
      if ( !get(driver, 'displayName') && intl.exists(key) ) {
        set(driver, 'displayName', 'key-'+ intl.t(key));
      }
    });
